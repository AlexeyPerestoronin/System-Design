...
## Что следует учесть при заработке каждого компонента серверной системы
1. DNS? - нужно ли отдельное доменное имя
0. Администрирование? - внутреннее или внешнее через выделенный сервис (возможно общий на систему) 
0. Мониторинг? - нужен ли отдельный сервис под мониторинг
0. Логирование? - нужен ли отдельный сервис под логирование
0. Аутентификация? - нужен ли отдельный сервис под аутентификацию пользователей системы (возможно, разный для разных пользователей)
0. Масштабирование? - нужно ли закладывать ресурсы на масштабирование системы
    1. Вертикальное - через увеличение мощности серверов
    0. Горизонтальное - через увеличения количества серверов
0. Кэширование? - критично ли кеширование и на какой стороне
    1. Client caching - расположение данных на машине клиента
    0. CDN caching (CDN = Content Delivery Network) - территориально-серверная оптимизация расположения данных
    0. Web server caching = Varnish - ускоритель HTTP
    0. Database caching - создание отдельных таблиц для быстрого доступа с определённым данным
    0. Application caching - кэширование данных времени выполнения
0. Load Balancer? - критично ли балансирование нагрузки и какими методами
    1. L-4: на уровне транспортных протоколов
    0. L-7: на уровне приложения
0. Database Prioritizing? - насколько критичны данные для работы сервиса или ключевым аспектом является логика работы
    1. [Выбор СУБД через классификацию](./data%20bases.md)
0. Какой способ интеграции с другими компонентами системы (внешними системами)
    1. File System (через файловую систему)
    0. DB (через базу данных)
    0. Web (при помощи какой-то сетевой технологии/паттерна)
        1. **REST API**
        0. **AsyncAPI - асинхронный или неблокирующий API** - интерфейс прикладного программирования, который позволяет (1) отправлять несколько запросов одновременно (2) и интеллектуально управлять соединениями между сервисами не дожидаясь ответа.
        0. **(Web)Socket** - принцип межсетевой коммуникации между системами, основанный на установлении устойчивого канала обмена данными на время соединения.  
        ...
        0. **SSE = Server-Sent Event** - технология отправки уведомлений от сервера к клиенту в виде DOM-событий.  
        ...
        0. **Long Request ~ Длинный опрос** - поход web-интеграции, при котором клиент, запрашивая сервер, получает немедленный ответ вместе с уникальным URL-адресом состояния, опрашивая который, клиент может получить актуальный статус-прогресс готовности результата запроса.
        0. **URL-обратного вызова** - подход web-интеграции, при котором клиент, отправив запрос, получает от сервера немедленный ответ с уникальным URL-адресом обратного вызова, по которому клиент может найти ответ, когда тот будет сформирован. Важно, что клиенту нет необходимости постоянно опрашивать полученный адрес, проверяя готовность, результата, т.к. сервер уведомит клиента, когда таковой результат будет доступен к получению.
        0. **Message Queue** - метод, при котором интеграция между компонентами системы происходит через очередь сообщений.
        0. **Message Broker** - метод, при котором интеграция между компонентами системы происходит через брокер сообщений, который берёт на себя множество посреднических функций:  
        ...
        0. **WebHook** - способ интеграции, когда два и более компонентов системы резервируют в режиме ожидания какой-либо канал связи с регистрацией обработчика события его активации.  
        ...
        0. **RPC = Remote Procedure Control** - класс технологий, позволяющих программам вызывать функции или процедуры в другом адресном пространстве (на удалённых узлах либо в независимой сторонней системе на том же узле).  
        ...
        0. **GraphQL** - это спецификация для языка запросов данных и языка манипулирования данными, предназначенный для построения web-ориентированных программных компонентов.  
